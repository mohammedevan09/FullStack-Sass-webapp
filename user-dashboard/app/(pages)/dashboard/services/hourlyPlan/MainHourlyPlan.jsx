...
const MainHourlyPlan = ({ services, pricing }) => {
  const [openModalCustom, setOpenModalCustom] = useState(false)
  const [openModal, setOpenModal] = useState(false)
  const [openSubModal, setOpenSubModal] = useState(false)
  const [pricingData, setPricingData] = useState(pricing[0])
            ...
            {pricing?.map((item, i) => (
              <div
                ...
                className={`flex justify-between sm:py-5 py-4 sm:px-6 px-4 rounded-xl border-2 cursor-pointer ${
                  pricingData?.title === item?.title
                    ? 'border-blue-800'
                    : 'border-white'
                }`}
                onClick={() => setPricingData(item)}
              >
                <div className="flex items-center sm:gap-6 gap-3">
                  ...
                  >
                    <CheckSignIcon3
                      color={pricingData?.title === item?.title && 'white'}
                    />
                  </div>
